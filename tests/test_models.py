...
def test_provider_model(init_database):
    ...
    pro = Provider.query.filter_by(name="Jack Reacher").first()

    assert pro is not None
    assert pro.trade == "Senior Software Engineer"
...
def test_provider_to_dict(init_database):
    ...
    assert pro_dict["name"] == "Jack Reacher"
    assert pro_dict["trade"] == "Senior Software Engineer"
...
def test_review_model_relationship(init_database):
    ...
    assert review.comment == "I highly recommend him for web development"
    assert review.provider.name == "Jack Reacher"
